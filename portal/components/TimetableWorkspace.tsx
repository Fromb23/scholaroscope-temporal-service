...
function WeeklyGrid({ calendar, entries }: { calendar: CalendarResponse | null; entries: VersionDetail["entries"] }) {
  ...
  return <div className="weekly-grid-wrap" tabIndex={0} aria-label="Weekly timetable grid"><div className="weekly-grid" role="grid" style={{ gridTemplateColumns: `150px repeat(${days.length}, minmax(180px, 1fr))`, gridTemplateRows: `52px repeat(${rows.length}, 92px)` }}>
    ...
    {entries.map((entry) => { const day = dayLabels.findIndex((label) => label.toUpperCase() === entry.day_of_week); const column = days.indexOf(day); const rowIndex = rows.findIndex((row) => row.slot_index === entry.start_period_index); if (column < 0 || rowIndex < 0) return null; const classLabel = entry.cohorts?.length ? entry.cohorts.map((cohort) => cohort.cohort_name).join(", ") : entry.cohort_name; return <article tabIndex={0} aria-label={`${entry.subject_name}, ${entry.teacher_name}, ${classLabel}, ${entry.start_time} to ${entry.end_time}`} className={lessonCardClass(entry)} key={entry.entry_uuid} style={{ gridColumn: column + 2, gridRow: `${rowIndex + 2} / span ${entry.duration_periods}` }}><strong>{entry.subject_name}</strong><span>{entry.teacher_name}</span><span>{classLabel}</span>{entry.delivery_group_uuid ? <small>Combined lesson{entry.learner_count ? ` · ${entry.learner_count} learners` : ""}</small> : null}{entry.parallel_block_uuid ? <small>Runs with alternatives</small> : null}{entry.room_name ? <span>{entry.room_name}</span> : null}<small>{clock(entry.start_time)}–{clock(entry.end_time)}{entry.duration_periods > 1 ? " · Double lesson" : ""}</small>{entry.has_hard_conflict ? <b>Conflict</b> : null}</article>; })}
  </div></div>;
}
