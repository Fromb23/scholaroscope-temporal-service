...
An assignment is the curriculum-independent edge `(workspace, academic year, term, teacher, cohort, canonical cohort subject, subject, optional resource)`. `weekly_periods` counts occupied cells; `double_blocks` is the subset that must occupy adjacent eligible cells. Rooms are optional.
...
2. Exact bipartite multigraph edge coloring for the weekly core. The graph is padded to a balanced delta-regular multigraph and decomposed into perfect matchings. This guarantees teacher/cohort exclusivity for the core within a bounded number of matching rounds.
...
- Combined cohort sessions are not enabled. Full coverage therefore requires one distinct eligible teacher per cohort-period.
